...
- `mds::is_forbidden_path_char` and `mds::escape_path_for_message`: the forbidden-path-character predicate (`is_control_char`'s 78-codepoint class plus `\n` and `\t` — 80 total) and its message-escaping helper, whose output carries none of the 80 (#265). The predicate is what the resolver, the built-in backends and the CLI now enforce (see **Changed**), and what a custom `FileSystem` backend must apply to the paths it produces itself.
- `mds::display_native_path`: the user-visible form of a filesystem path — strips a Windows verbatim `\\?\` prefix wherever that is lossless, a no-op off Windows. The entry point for displaying a path to a user (the CLI's status lines and error messages go through it); internal comparisons keep the canonical/verbatim form unchanged (#409).

### Changed

- **BREAKING (Rust API): `FileSystem::normalize` is replaced by the required `FileSystem::resolve_entry`, and `FileSystem::canonicalize` + `FileSystem::set_root` by `FileSystem::anchor_base_dir` (#155).** `normalize(base, relative)` mixed two jobs: with `base == ""` it resolved an entry path (and, on `NativeFs`, anchored the project root), and with a non-empty `base` it resolved an import — a branch no production caller ever used. Entry resolution is now `resolve_entry(path)`; import resolution stays `normalize_in_dir(parent_dir(key), relative)`. Custom backends passed to `ModuleCache::with_fs` must implement `resolve_entry` instead of `normalize` (return the key unchanged for an in-memory backend). The resolver now validates every entry path itself before calling the backend, so a custom backend never receives an empty entry path or one containing a NUL byte. `NativeFs` now enforces the documented 256-segment cap on entry paths and imports (only `VirtualFs` did before). An entry path that is empty or contains a NUL byte now reports `mds::io` (was `mds::import`), with the path escaped in the message; a NUL byte in an `@import` string is still `mds::import`. The base directory of `ModuleCache::resolve_source*` now goes through one call, `anchor_base_dir(dir)`, whose default returns `dir` unchanged; on `NativeFs` it canonicalizes the directory, refuses one whose final component is a symlink, and anchors the project root at the first call (a later call never moves it) — `set_root` anchored the root without the symlink check. `ModuleCache::resolve_key` now validates its key and resolves it through `resolve_entry` like every other entry API: on `NativeFs` a symlinked key is refused, the first key anchors the project root — so containment now applies to the imports of a `resolve_key` entry, where before no root was ever set — and a later key outside that root is refused. `ModuleCache::resolve_virtual_intrinsic`/`_opts`, and with them `compile_virtual`, `check_virtual`, `lint_virtual` and the WASM binding, validate the entry key too: an empty key or one containing a NUL byte is `mds::io` and a key of more than 256 segments is `mds::resource_limit`, even when the module map contains that key. `mds fmt` and `mds lint` report a failure to anchor a file's project root (`mds::io`, exit 2) instead of ignoring it.

- **BREAKING: a path carrying any of the 80 forbidden path characters is refused at input (#265).** The class is `mds::is_forbidden_path_char` — C0 controls including `\n` and `\t`, DEL, C1 controls and the bidi/format hazards (U+061C, U+200E/U+200F, U+2028/U+2029, U+202A–U+202E, U+2066–U+2069, U+FEFF). An import string carrying one — `@import`, `@export … from`, `@extends` or a frontmatter `imports:` path — is `mds::import` (`import path contains forbidden character U+XXXX: "<path>"`); an entry path or virtual entry key (`compile_virtual`, `check_virtual`, `lint_virtual`, `ModuleCache::resolve_key`/`resolve_virtual_intrinsic*`, and so the WASM `filename`), the base directory of a string compile (`compile_str_with`/`check_str_with`/`lint_str_with`, the bindings' `basePath`/`base_path`, `ModuleCache::resolve_source*`), and a path whose resolved canonical form carries one — a symlinked directory leading into a hostile-named directory — are `mds::io`. The resolver runs these checks before any `FileSystem` backend is called, so a custom `with_fs` backend is covered; `NativeFs` also scans every canonical path it resolves, and `NativeFs::check_symlink` refuses such a path as well. The formatter (`format_str_with`/`format_str_named`, and so `mds fmt`) resolves its base directory exactly as `check_str_with` does and fails with the same error when that directory is refused or cannot be resolved, instead of skipping its compile-equivalence check and formatting anyway (`mds fmt -` in a hostile-named working directory exited 0 where `mds check -` exited 2). Every message names the codepoint as `U+XXXX`, shows the path the caller typed escaped with `mds::escape_path_for_message` (never the absolute resolved path), and so carries no forbidden character itself; a NUL byte keeps its own `contains null byte` message. A frontmatter `imports:` path error now names the rule the path actually breaks — `must start with './' or '../'`, `contains null byte` or `contains forbidden character U+XXXX` — instead of reporting `must start with './' or '../'` for all three. A project located under a directory whose name carries a forbidden character now fails entirely, by design. On the CLI: the directory walker still collects a hostile-named file, and that one file fails (`mds::io`, the name shown escaped) while its siblings are processed, with each subcommand's existing per-file exit code (`build`/`check`/`fmt` 1, `lint` 2), and `mds watch` keeps running, naming the file as walked from the directory as typed (`src/…`), as `mds build` does, rather than by its canonical absolute path; `-` (stdin) resolves imports against the working directory, and under `build`, `check`, `lint` and `fmt` a working directory whose path carries one is refused as `resolved path contains forbidden character U+XXXX: "."` (the message used to show the absolute working directory); a `mds lint`/`mds fmt` file argument carrying one is refused before its existence and extension checks, so a missing or non-`.mds` hostile path is `mds::io` rather than `file not found`/`not an MDS file` (whose messages would show it unescaped); `-o`/`--output`, `--out-dir`, `mds.json` `build.output_dir` and `mds init <filename>` carrying one are refused up front, before any input is read (`mds::io`, exit 2), and `-o`/`--output`, `--out-dir` and `build.output_dir` are refused up front as well when the location they resolve to carries one — a symlink, or a working directory above a relative value, leading into a hostile-named directory (`<setting> resolved path contains forbidden character U+XXXX: "<value as written>"`) — so nothing is created or written there and no later status line shows it; CLI status lines escape a `\t` in a path too, like every other forbidden character — the `mds.json` check runs when the config is loaded, so it fails every subcommand that loads `mds.json` (`build`, `watch`, `lint`, and `fmt` in directory mode; `check` does not load it). An `mds.json` load error (`invalid mds.json at …`, `mds.json at … is too large`, `invalid UTF-8 in …`, `cannot read …`) names the file by the path the input leads to it (`./mds.json`, `sub/../mds.json`), escaped, instead of by its canonical absolute path — which, because the config loads before the input is validated, printed a raw TAB from a hostile-named directory above the project. A `build.output_dir` containing a `..` component is now `mds::io`, exit 2 (was exit 1), with the value escaped in the message. `mds watch --vars` reports the real reason a `--vars` path is refused (a forbidden character, a file removed since startup) instead of always saying `--vars file must not be a symlink`, and refuses a `--vars` path carrying a forbidden character before it probes it or watches its directory (directory mode used to watch it first, and its `failed to watch vars directory` warning showed the name with a raw TAB). The napi, WASM and Python bindings throw these errors with the same codes and messages (an `mds::import` message carries the usual `import error: ` prefix), so a `lintVirtual`/`lint_virtual` whose import names a hostile module now throws instead of returning a lint result. A module key a binding's own options error names — a non-string or oversized `modules` entry, and the WASM `filename_collision` — is escaped with `mds::escape_path_for_message` too, in the same `modules["<key>"]` form on all three (napi and WASM showed it raw; Python showed a Rust debug string). `@mdscript/mds`'s WASM backend, whose `compileFile`/`checkFile`/`lintFile` read the entry file and its imports in JS before the engine runs, applies the same class in that pre-scanner before opening any file, and fails with exactly the error the native backend throws for the same input: an entry path that is empty, contains a NUL byte or carries a forbidden character is `mds::io`; an import string that is not `./`/`../`-relative, contains a NUL byte or carries a forbidden character is `mds::import` (the pre-scanner used to open a non-relative import and fail with a raw `ENOENT` when it did not exist, and reported a NUL byte or an empty import as an uncoded `security: …` error); a file whose resolved path carries one is `mds::io`.

### Removed

- **BREAKING (Rust API): `mds::fix::apply_fixes` is removed (#304).** It was deprecated since 0.4.0; use `mds::fix::apply_fixes_incremental`, which applies the same reverify gate batch-first and then retries each edit on its own, so its closure must be `Fn` rather than `FnOnce` and it can return `FixOutcome::PartiallyFixed` (a `_ => {}` arm silently discards that). The six ADR-004 reverify-gate regression tests that ran only through `apply_fixes` now run against `apply_fixes_incremental`.

### Fixed

- **Security:** on the native filesystem, an entry, `@import`/`@extends` target or base directory whose resolved path is not valid UTF-8 — reachable through a symbolic link into a directory whose name is not, which Linux allows in a checked-out repository — is refused (`mds::io`, `resolved path is not valid UTF-8: "<path as written>"`). `NativeFs` used to key it by its lossy form, which replaces each invalid sequence with U+FFFD and so names a different, valid-UTF-8 path: every check — symlink, root containment, forbidden characters — ran on the real path, and the file then read was the "twin", which could itself be a symbolic link to any file on the host. The key is now the resolved path's exact UTF-8 form or nothing.
- A string compile, check or lint whose base directory is a filesystem root (`/`, or a Windows drive root) failed with `cannot resolve path /: file not found: /` before touching the filesystem — the root has no final path component for the symlink check to inspect — and now resolves from that root, never from the current working directory (#371). Affected: `mds::compile_str_with`/`check_str_with`/`lint_str_with`; CLI stdin (`mds build/check/lint -`), whose base directory is the process's working directory; the napi and Python bindings' `basePath`/`base_path`; and a native `@mdscript/mds` string compile with no `basePath` in a process whose working directory is `/` — the case the release pipeline's Alpine load test hit, since `node:22-alpine` sets no `WORKDIR`. `mds fmt` on a file directly under a filesystem root no longer silently downgrades its safety gate to the weaker structural-equivalence fallback: the compile-and-diff check runs, as for any other file.
- On a case-insensitive volume (the macOS default, and NTFS on Windows), an `@import` or entry path whose case differs from the file's on-disk name — `./Header.mds` for `header.mds`, `mds build Entry.mds` for `entry.mds` — no longer fails with a false `symlinks are not allowed in imports` error. Symlink detection now reads the file type of the final path component instead of comparing its canonical path with the path as written, so a real symbolic link (and, on Windows, a junction) is still refused under any spelling. The module is keyed by its on-disk spelling, so importing one file under two spellings loads it once. The `@mdscript/mds` WASM backend's `compileFile`/`checkFile`/`lintFile` had the same false positive in their JS pre-scanner (`… resolved to unexpected location … — possible symlink`) and now apply the same rule: the final component is judged by its own file type, so a real symlink (and on Windows a junction) is still refused under any spelling; a symlinked parent directory is followed, as the native backend follows it; and containment in the project root is checked on the canonical path. Its virtual filesystem resolves imports by name, from the importing module's key, so an import that leaves a symlinked directory through `..` — which the native backend resolves beside the link's target, and the engine beside the link — is refused (`mds::import`, `import path leaves a symlinked directory through '..', which the WASM backend cannot resolve: "<path>"`) rather than compiled from a file the native backend never reads. That virtual filesystem is also case-sensitive, so there each spelling stays its own module key — same output, the file read once per spelling — and a file reached both directly and through a symlinked directory is read once per key (#408).
- `@mdscript/mds`'s WASM backend (`compileFile`/`checkFile`/`lintFile`) reports a missing entry file or import — including a case-mismatched spelling on a case-sensitive volume, a directory that does not exist, and a path running through a regular file where a directory is expected — as `mds::file_not_found` with the message `file not found: <path as written>`, matching the native backend. So is every other path it cannot resolve — a symlink loop, a name too long, a directory that cannot be searched — as the native backend reports them, and a file that resolves but cannot be read is `mds::io` (`cannot read <path as written>: <errno name>`, e.g. `EACCES`; the native message names its root-relative path and the OS error text instead). Its JS pre-scanner used to surface Node's raw, uncoded errors (`ENOENT`, `ENOTDIR`, `ELOOP`, `ENAMETOOLONG`, `EACCES`, …), which named the absolute resolved path — with the raw characters of a hostile-named working directory in it. Its own refusals now carry the native error too, keyed on the path as written: a symlinked entry or import is `mds::import` (`symlinks are not allowed in imports: <path>`), an import outside the project root is `mds::import` (`import path escapes project directory: "<path>"`), and a module that is not a regular file is `mds::io` (`cannot read <path>: EISDIR` for a directory) — they were uncoded `security: …` errors naming the absolute resolved path. The pre-scanner checks an import's containment before it touches the filesystem, so a MISSING file outside the project root is refused as escaping it where the native backend reports `file not found` (#408).
- On Windows, the `dependencies` of a native compile — the Rust `CompileResult`, the napi and `@mdscript/mds` native-backend `compile()`/`compileFile()` results, and Python's `CompileResult.dependencies` — no longer carry the verbatim `\\?\` prefix that path canonicalization adds: `\\?\C:\…` is reported as `C:\…` and `\\?\UNC\server\share\…` as `\\server\share\…`. A path the conventional form cannot name exactly — longer than `MAX_PATH`, or with a component such as a reserved device name (`CON`, `nul.txt`) or a trailing dot or space — keeps the prefix. `mds watch` keeps comparing dependencies in canonical form. The CLI's own status lines and error messages carried the same prefix whenever a canonicalized `--out-dir` (directory-mode `build`/`watch`) reached a `Compiled to …`, `Removed stale …`, or `atomic_write_file` I/O error message; they now show the conventional form too, through a new public `mds::display_native_path` (a no-op off Windows) that the CLI's single path-display choke-point, `safe_path`, now routes every status-line path through (#409).
- An error raised inside inherited `@extends` content is reported against the file it is written in — the base template or the child — with source maps off and for an extending module reached through `@import`, as it already was with source maps on: an inherited `mds::type_mismatch` carried no span and now carries one on the base, so the serialized error no longer depends on the source-map option. On the native backend, a validation error in inherited content (e.g. an undefined variable in a base default block) also named the base by its absolute path — in the CLI's rendered frame and in `MdsError::source_name()` — and now names it root-relative like every other diagnostic (#114).
- In a messages-mode `@extends` chain, an error in inherited content — orphan text outside every `@message` (`mds::mixed_content`) or a cross-type comparison — is reported against the base template it is written in. The whole chain was evaluated against the child's source, so the span landed on an unrelated line of the child, or, when the base offset ran past the child's end, carried no source at all (#115).

### Internal

- **Windows Rust CI (#147).** A new `rust-windows` job (`Rust — clippy, test (windows-latest)`) runs `cargo clippy --workspace --all-targets -- -D warnings` and `cargo test --workspace --no-fail-fast` (doctests included) on `windows-latest`. It is listed in `scripts/verify-pr-checks.mjs`'s `EXPECTED_CONTEXTS` (Tier A+, now five entries), so the pre-merge verifier requires it, and it becomes the 16th required context in `main`'s branch protection when this change merges (procedure: `RELEASING.md` "Adding a required CI context"). The Windows gaps it surfaced are fixed rather than gated: symlink-creating tests use a cross-platform `make_symlink` helper (skipping only on an unprivileged Windows with `CI` unset, never silently; one copy per test compilation unit), `source_path.rs`'s platform-conditional component comparison is one shared `comp_eq`, and `dir_build.rs`'s `build_file` helper is `#[cfg(unix)]` like its only caller. 26 tests that were `#[cfg(unix)]` without needing to be now run on Windows too: the 24 that were gated only because they create a symlink — `crates/mds-cli/src/output.rs` (2), `crates/mds-cli/tests/{cli_build,cli_commands,cli_fmt,cli_source_map,cli_watch,dir_build,intrinsic_output,security}.rs` (20 across the eight files), `crates/mds-core/src/lib.rs` (1) and `crates/mds-core/tests/api_surface.rs` (1) — plus `cli_fmt.rs`'s `bad_utf8_exits_two`, whose invalid UTF-8 is in the file content, not its name, and `crates/mds-cli/src/lint.rs`'s `sort_key_sanitizes_control_byte_filenames`, whose control-character path is only built in memory. Every `#[cfg(unix)]` gate left in the touched files carries a one-line reason (Unix permission bits, chmod-provoked write failures, `SIGINT`, non-UTF-8 filenames built with `OsStr(ing)Ext`, control-character filenames Windows cannot create).
- **`apply_fixes_incremental` decomposed (#208).** The public function is now a 30-line pipeline over single-purpose private helpers — `precheck` (overlap, then empty plan, then sortedness), `RegressionGate` and `Verdict` (the reverify gate, built once per plan and judged against the full set of targeted rules), `splice_edit` (shared with `apply_plan_unchecked`), `apply_per_edit`, `summarize_rejections` and `fallback_cap_rejected` — with no behaviour change. New pins hold the exact overlap, sortedness, fallback-cap and joined-rejection messages, the per-edit residual semantics, and the batch-success-above-cap path.
- **`unreachable-branch`'s `check_if_block` split into helpers (#235).** `check_primary`, `primary_true_fix`, `primary_false_fix`, `check_elseifs`, `next_boundary`, `elseif_diag`, `constant_elseif_finding`, `exclusive_removal` and `push_or_break` replace one 182-line function; every function in the file is at most 40 lines. Golden tests landed first and pass unchanged: every message, help text, span and fix span for cases A–G, the seen-conditions semantics (three identical conditions yield two duplicate findings) and diagnostic-cap truncation.
- **One evaluation budget per `@extends` chain (#114).** The evaluator's cumulative budgets — loop iterations and message bytes — are one `EvalBudget` threaded through seeded entry points, so an `@extends` chain evaluated region by region spends a single budget, as a module with one body does, with source maps on or off and in messages mode; `splice_skeleton` and `ExtendsComponents::final_body` are deleted.
- **Docs.** `spec.md` §4.6 gains a forbidden-characters row and a table of where each check runs, with its error code and the tests that pin it, and states the one residual (a custom `FileSystem` backend's own paths); the §5 error-code rows, §7.2, §7.6, §7.8 and §7.9 follow. `SECURITY.md` describes input refusal alongside output escaping. The `FileSystem` trait's rustdoc "Security Contract" separates what the resolver checks for every backend from what a custom backend must check itself. `mds::scan_imports`' rustdoc states that its output is the raw, unvalidated import strings (#265). `RELEASING.md` documents adding a required CI context (#147); `CLAUDE.md` gains the Windows cross-clippy command and two stale-artifact traps.
...
[Unreleased]: https://github.com/dean0x/mdscript/compare/v0.4.4...HEAD
[0.4.4]: https://github.com/dean0x/mdscript/compare/v0.4.3...v0.4.4

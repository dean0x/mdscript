...
- `@mdscript/mds`'s WASM backend (`compileFile`/`checkFile`/`lintFile`) reports a missing entry file or import — including a case-mismatched spelling on a case-sensitive volume, a directory that does not exist, and a path running through a regular file where a directory is expected — as `mds::file_not_found` with the message `file not found: <path as written>`, matching the native backend. Its JS pre-scanner used to surface Node's raw, uncoded `ENOENT`/`ENOTDIR` error, which named the absolute resolved path (#408).

...
- Linux cross-builds: only x86_64-gnu uses napi `--use-napi-cross`; aarch64-gnu uses the apt cross gcc; BOTH musl legs use zig cc wrappers, because `--use-napi-cross` has no musl toolchain and napi forces `-crt-static` on musl, so the host gcc silently links glibc (v0.1.0-v0.3.0 shipped a glibc-linked linux-x64-musl addon). release.yml gates every musl artifact with readelf.

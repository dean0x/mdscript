...
- On a case-insensitive volume (the macOS default, and NTFS on Windows), an `@import` or entry path whose case differs from the file's on-disk name — `./Header.mds` for `header.mds`, `mds build Entry.mds` for `entry.mds` — no longer fails with a false `symlinks are not allowed in imports` error. Symlink detection now reads the file type of the final path component instead of comparing its canonical path with the path as written, so a real symbolic link (and, on Windows, a junction) is still refused under any spelling. The module is keyed by its on-disk spelling, so importing one file under two spellings loads it once. The `@mdscript/mds` WASM backend's `compileFile`/`checkFile`/`lintFile` had the same false positive in their JS pre-scanner (`… resolved to unexpected location … — possible symlink`) and now apply the same rule: the final component is judged by its own file type, so a real symlink (and on Windows a junction) is still refused under any spelling; a symlinked parent directory is followed, as the native backend follows it; and containment in the project root is checked on the canonical path. Its virtual filesystem is case-sensitive, so there each spelling stays its own module key — same output, the file read once per spelling (#408).

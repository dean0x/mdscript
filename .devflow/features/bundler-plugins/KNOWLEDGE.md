...
- The `LazyInit` in `createMdsLoader` clears `pending` on rejection — a failing `esmImport()` will be retried on the next loader invocation. `capturedOptions` (and `capturedOptionsJson`) are set BEFORE the async factory runs (loader.ts line ~110-111), so after a failed first init `capturedOptions` is non-null and the option-drift warning IS reachable. If the second invocation has different options, the warning fires and the original options remain bound in the factory closure. This is an edge case of the retry behavior — first-options-win holds regardless of init failure.

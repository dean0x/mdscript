/**
 * Contract tests for @mdscript/rspack-loader.
 *
 * Non-HMR tests only — verifies loader emits correct module output with
 * all three required symbols (default, _resetForTesting, _setTransformerForTesting).
 *
 * The rspack HMR lifecycle spec is added in Coder 2 (Step 7).
 * Tests are cross-OS, no watcher, no filesystem events.
 */
...
describe('mdsRspackLoader', () => {
  ...
  test('exports _resetForTesting', () => {
    assert.equal(typeof _resetForTesting, 'function');
  });

  test('exports _setTransformerForTesting', () => {
    assert.equal(typeof _setTransformerForTesting, 'function');
  });

  test('loader returns compiled module with export default', async () => {
  ...
  test('loader returns module with metadata export', async () => {
    const ctx = createLoaderContext(SIMPLE_MDS);
    await mdsLoader.call(ctx);

    assert.equal(ctx.callbackResult.err, null);
    assert.ok(
      ctx.callbackResult.content.includes('export const metadata'),
      'content should have metadata export',
    );
  });

  test('loader calls addDependency for file imports', async () => {
  ...
  test('emitWarning called once per compiler warning', async () => {
    ...
    assert.ok(ctx.emittedWarnings[0] instanceof Error);
    assert.ok(ctx.emittedWarnings[1] instanceof Error);
    assert.equal(ctx.emittedWarnings[0].message, 'first warning');
    assert.equal(ctx.emittedWarnings[1].message, 'second warning');
  });

  test('no HMR runtime injected — emitted module contains no import.meta.webpackHot', async () => {
    // Decision D1/G2: the emitted module must NOT contain any HMR self-accept
    // footer. HMR is handled at the bundler level, not injected into the module.
    const ctx = createLoaderContext(SIMPLE_MDS);
    await mdsLoader.call(ctx);

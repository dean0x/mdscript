...
7. **Reverse mutation control (NEW, mandatory):** delete only the `#[deprecated(...)]` attribute, run `cargo clippy --workspace --all-targets -- -D warnings` (reports 10 fix.rs expectations; the workspace run aborts before the `api_surface` integration-test target is compiled, observed per api_surface.rs:1670-1673), then also run `cargo clippy -p mds-core --test api_surface -- -D warnings` (reports 1 api_surface.rs expectation); together confirm **exactly 11** `unfulfilled_lint_expectations` across both commands (applies ADR-009), then restore both and confirm clean. Without this, AC-209-05 is an absence-only claim.
...
- **Expected outcome:** The mutated workspace run MUST exit non-zero with exactly 10 `unfulfilled_lint_expectations` diagnostics anchored in fix.rs at the ten `#[expect(deprecated)]` fn-decl lines (the workspace run aborts before the `api_surface` integration-test target is compiled, observed behavior per api_surface.rs:1670-1673, applies ADR-009). The mutated targeted run (`-p mds-core --test api_surface`) MUST exit non-zero with exactly 1 `unfulfilled_lint_expectations` diagnostic anchored at the `F-API-3` test in api_surface.rs. Together the two commands produce exactly 11 diagnostics. Both restored runs MUST exit 0. If the combined count is fewer than 11, the pin is weaker than claimed and AC-209-05 fails.
- **Verification method:** integration

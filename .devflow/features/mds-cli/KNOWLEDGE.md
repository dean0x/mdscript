...
- Forbidden path characters (#265) are refused UP FRONT, before any input is read: `-o`/`--output` and `--out-dir` by `build::reject_forbidden_output_flags` (build and watch), `mds.json build.output_dir` inside `load_config` (so it fails every subcommand that loads the config: `build`, `watch`, `lint`, directory-mode `fmt`), and `mds init <filename>` in `main.rs` — all via `output::reject_forbidden_output_path` (`mds::io`, exit 2, value shown through `mds::escape_path_for_message`), checking the value AS TYPED. `-o`/`--out-dir`/`build.output_dir` also each run `output::reject_forbidden_resolved_output_path` (1e73cbc): it canonicalizes the deepest EXISTING ancestor of the value (`resolve_existing_prefix` — a relative value resolves against the working directory; `None` when nothing on disk resolves) and scans the WHOLE resolved form, so a symlink — or, for a relative value, a hostile-named working directory above it — is refused before the location is ever created or written (`<setting> resolved path contains forbidden character U+XXXX: "<value as written>"`, never the absolute resolved path; `-o -` is stdout and is exempt). A hostile-named FILE is different: the directory walker still collects it and it fails on its own (`mds::io`) while siblings are processed; `watch` keeps running. Every directory-mode subcommand names that file as walked from the root AS TYPED (`src/evil\u…` for `src`) — `watch` included since a3ec639 (it used to show the canonical absolute path). Pinned by `crates/mds-cli/tests/forbidden_paths.rs`.
- `watch.rs` directory mode carries the root in two forms, `WatchRoot { canonical, typed }` (a3ec639, #265). `canonical` is what notify reports event paths under, so event filtering, graph keys (`graph_key`), `collect_mds_files`, output mapping (`output_path_for`/`output_base_no_ext`) and containment (`starts_with`) all use it; every COMPILE — startup, dedup baseline, `compile_one_source`, and the dep-refresh of an excluded in-root file — goes through `WatchRoot::walked(src)` = `typed` joined with `src`'s path below `canonical`, the form `build` walks, so errors never show the canonical absolute path. An out-of-root dependency has no walked form and is compiled by its canonical path. The graph is unaffected: `compile_with_deps_opts` returns canonical deps and filters the entry by its canonical form. File mode (`run_watch_file`) still compiles the canonical entry; its startup refusal of a hostile file argument names the typed path (`watch_file_mode_names_the_hostile_file_as_typed`).
- `mds watch .` (likewise `..` and `/`) cannot start: `run_watch` runs `NativeFs::check_symlink` on the typed input, and `check_symlink_named` returns `file not found: .` for a path whose `file_name()` is `None`. Pre-existing on `main`, not a #265 regression; `mds build .` walks the typed directory and is unaffected. Watch a named directory (`watch src`) until it is fixed.
...
- `crates/mds-cli/tests/forbidden_paths.rs` — #265 walker matrix (build/check/fmt/lint/watch), single-file-argument refusals, output-location (typed and resolved) and `mds init` refusals, stdin `"."` cwd refusal, `--vars` message; `watch src` names a hostile file `src/…` and never quotes the canonical path, file mode names it as typed
- `crates/mds-cli/src/watch.rs` — watch loop; uses `compiled.kind.extension()` and `probe_and_remove_stale`; directory mode compiles through `WatchRoot::walked`

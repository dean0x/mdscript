/**
 * Real-driver end-to-end HMR / watch tests for @mdscript/rspack-loader.
 *
 * These tests drive an ACTUAL rspack watcher and assert on freshly emitted
 * bundle content read from disk — not mocked loader calls. They verify the
 * full pipeline: edit .mds file → rspack re-invokes loader → fresh bundle.
 *
 ...
 *   - rspack 1.7.11: stats shape is webpack-5-compatible for .hasErrors() and
 *     .toJson().errors. Bundle is written to output.path on disk.
 *   - rspack uses import.meta.webpackHot for ESM HMR (loader must not inject it).
...
// Absolute path to the CJS loader (rspack requires CJS loaders, same as webpack)
...
/**
 * Start an rspack watcher and wait for the first callback.
 * Returns { watching, getBundle } where getBundle() reads the latest build.
 *
 * rspack stats shape: compatible with webpack 5 — .hasErrors(), .toJson().errors[].message
 * (verified against rspack 1.7.11).
 *
 * @param {import('@rspack/core').Configuration} config
 * @returns {Promise<{ watching: object, getBundle: () => string, hasErrorState: () => boolean }>}
 */
async function startRspackWatcher(config) {
  ...
  await waitFor(
    () => latestStats !== null,
    { timeoutMs: 30_000, intervalMs: 100, label: 'rspack first build' },
  );
  ...
  return { watching, getBundle, hasErrorState, getStats: () => latestStats };
}

// ---------------------------------------------------------------------------
// Suite 1: HMR lifecycle (real rspack watcher)
// ---------------------------------------------------------------------------

describe('rspack-loader HMR e2e — Suite 1 (real watcher)', { skip: !HMR_ENABLED && 'HMR e2e tests are Linux-gated; set MDS_HMR=1 to run' }, () => {
  ...
  test('T-HMR-b (AC-F2): edit transitive @import dep → fresh bundle', async () => {
    ...
    try {
      ...
      assert.ok(getBundle().includes('MARKER_B'), 'dep edit propagates through rspack');
    } finally {
      await new Promise((resolve) => watching.close(resolve));
      cleanup();
    }
  ...
  test('T-HMR-c (AC-F3): inject compile error → rspack surfaces error, watcher stays alive', async () => {
    ...
    try {
      ...
      assert.ok(latestStats.hasErrors(), 'rspack entered error state');
      // T-C4: error info in stats
...
// ---------------------------------------------------------------------------
// Suite 3: Edge cases (real rspack watcher, Linux-gated)
// ---------------------------------------------------------------------------
...
describe('rspack-loader HMR e2e — Suite 3 edge cases', { skip: !HMR_ENABLED && 'HMR e2e tests are Linux-gated; set MDS_HMR=1 to run' }, () => {
  ...
  test('T-E-mdflip (AC-E2): .mds file compiles regardless of frontmatter presence', async () => {
    // rspack: shouldTransform() for .mds always returns true (extension-based).
    // Content without MDS frontmatter is treated as plain text template.
    // This test documents: any .mds file compiles in rspack mode.
    const { dir, paths, cleanup } = createTempMdsProject({
      'entry.mds': 'Just plain text. MD_PLAIN_MARKER',
    });
    const outDir = join(dir, 'out');
    const { watching, getBundle } = await startRspackWatcher(buildConfig(paths['entry.mds'], outDir));
  ...
  test('T-E-cycle: circular @import → rspack errors, no infinite rebuild loop', async () => {
    ...
    try {
      ...
      assert.ok(
        ...
        `Circular @import caused ${countAfter - countBefore} extra rspack callbacks — possible infinite loop`,
      );
    } finally {
      await new Promise((resolve) => watching.close(resolve));
      cleanup();

...
- Release-surface PR gate: `release.yml` now triggers on `pull_request` events touching `.github/workflows/release.yml`, `.github/actions/**`, `crates/mds-napi/**`, `crates/mds-python/**`, and `scripts/verify-napi-names.mjs`; adds `rehearse-publish-python` job (OIDC dry-run, unguarded per PF-039) and `publish-testpypi` opt-in leg; GHCR manifest probe in `version-gate` for the `pypa/gh-action-pypi-publish` pin (PF-040, #350); CI-history step guard is now step-level (ADR-013 amendment); `verify-pr-checks.mjs` requires `Rehearse PyPI publish (no upload)` on release-surface PRs (#342, #350).

...
- **Windows Rust CI (#147).** A new `rust-windows` job (`Rust — clippy, test (windows-latest)`) runs `cargo clippy --workspace --all-targets -- -D warnings` and `cargo test --workspace --no-fail-fast` (doctests included) on `windows-latest`. It is listed in `scripts/verify-pr-checks.mjs`'s `EXPECTED_CONTEXTS` (Tier A+, now five entries), so the pre-merge verifier requires it, and it becomes the 16th required context in `main`'s branch protection when this change merges (procedure: `RELEASING.md` "Adding a required CI context"). The Windows gaps it surfaced are fixed rather than gated: symlink-creating tests use a cross-platform `make_symlink` helper (skipping only on an unprivileged Windows with `CI` unset, never silently; one copy per test compilation unit), `source_path.rs`'s platform-conditional component comparison is one shared `comp_eq`, and `dir_build.rs`'s `build_file` helper is `#[cfg(unix)]` like its only caller. 25 tests that were `#[cfg(unix)]` without needing to be now run on Windows too: the 24 that were gated only because they create a symlink — `crates/mds-cli/src/output.rs` (2), `crates/mds-cli/tests/{cli_build,cli_commands,cli_fmt,cli_source_map,cli_watch,dir_build,intrinsic_output,security}.rs` (20 across the eight files), `crates/mds-core/src/lib.rs` (1) and `crates/mds-core/tests/api_surface.rs` (1) — and `cli_fmt.rs`'s `bad_utf8_exits_two`, whose invalid UTF-8 is in the file content, not its name. Every `#[cfg(unix)]` gate left in the touched files carries a one-line reason (Unix permission bits, chmod-provoked write failures, `SIGINT`, non-UTF-8 filenames built with `OsStr(ing)Ext`).

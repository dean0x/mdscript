...
7. **Reverse mutation control (NEW, mandatory):** delete only the `#[deprecated(...)]` attribute, run `cargo clippy --workspace --all-targets -- -D warnings`, confirm **exactly 11** `unfulfilled_lint_expectations` (10 fix.rs + 1 api_surface.rs — the lib rlib compiles clean without the attribute, so both the lib-test and integration-test targets compile concurrently and each report their expectations in the same workspace run, applies ADR-009), then restore and confirm clean. Without this, AC-209-05 is an absence-only claim.
...
5. AC-209-05 (functionality, positive, mutation-verified): With the implementation complete, deleting ONLY the `#[deprecated(...)]` attribute from `apply_fixes` (leaving the function and all suppressions in place) MUST cause `cargo clippy --workspace --all-targets -- -D warnings` to FAIL with exactly 11 `unfulfilled_lint_expectations` diagnostics — 10 in `crates/mds-core/src/lint/fix.rs` and 1 in `crates/mds-core/tests/api_surface.rs`. The workspace command reports all 11 because removing the attribute leaves the lib rlib compiling clean; both the lib-test (fix.rs `#[cfg(test)]`) and the integration-test (api_surface) targets become ready simultaneously and cargo schedules them concurrently, so both targets emit their diagnostics in the same run (applies ADR-009). Two commands are required (applies ADR-009, observed behavior per api_surface.rs:1670-1673): `cargo clippy --workspace --all-targets -- -D warnings` reports 10 fix.rs expectations and aborts before the `api_surface` integration-test target is compiled; `cargo clippy -p mds-core --test api_surface -- -D warnings` reports the 1 api_surface.rs expectation individually. Restoring the attribute MUST restore a clean run on both commands. If the combined count is fewer than 11, the pin is weaker than claimed and AC-209-05 fails.
...
- **Setup:** On a clean tree at the final implementation state, delete only the `#[deprecated(...)]` attribute block above `pub fn apply_fixes` in `crates/mds-core/src/lint/fix.rs`. Run `cargo clippy --workspace --all-targets -- -D warnings`. Then `git checkout -- crates/mds-core/src/lint/fix.rs` and re-run the same command.
- **Expected outcome:** The mutated workspace run MUST exit non-zero with exactly 11 `unfulfilled_lint_expectations` diagnostics — 10 anchored in fix.rs at the ten `#[expect(deprecated)]` fn-decl lines and 1 anchored at the `F-API-3` test in api_surface.rs. The workspace command reports all 11 because removing the attribute leaves the lib rlib compiling clean; both the lib-test (fix.rs `#[cfg(test)]`) and the integration-test (api_surface) targets become ready simultaneously and cargo schedules them concurrently, so both emit their diagnostics in the same run (applies ADR-009). The workspace command does NOT abort before the api_surface target is compiled — that claim is build-scheduling dependent and not a property of the code. The restored run MUST exit 0. If the mutated run reports fewer than 11, the pin is weaker than claimed and AC-209-05 fails.
- **Verification method:** integration

...
- **The scanner's own refusals are NativeFs's** (be14535), each keyed on `shown`, the path as written: a symlinked final component, entry or import alike → `symlinkError` (`mds::import`, `symlinks are not allowed in imports: <path>`), from the `O_NOFOLLOW` ELOOP, the post-open `lstat`, and a component swapped after the checks (canonical dir ≠ canonical parent); a module outside the project root → `escapesProjectError` (`mds::import`, `import path escapes project directory: "<path>"`), from both the lexical pre-check in `validateImportPath` and the canonical containment check; a module that opened but is not a regular file → `mds::io` `cannot read <path>: EISDIR` for a directory (the errno reading it reports; Windows fails the open itself with its own errno), `not a regular file` for a device/FIFO/socket — NativeFs fails such a module at its read step. `cannotReadError(shown, reason)` is the single `cannot read …` builder; `readError(shown, err)` delegates with the errno name. **Ordering residual**: containment is checked lexically BEFORE anything outside the root is touched, so a MISSING file outside the root is `escapes project directory` on WASM where native reports `file not found` (native resolves the file first — an existence oracle the scanner deliberately does not replicate).
- **Every filesystem error is coded, never re-thrown raw** (a6d1673, closing most of #408's #414 gap): `openNoFollow` catches `ELOOP` as the symlink refusal (see Symlinks below); every OTHER open failure is classified by whether the path can still be `lstat`ed — resolvable but unopenable (e.g. `EACCES` on the file itself) → `readError(shown, err)` (`mds::io`, `cannot read <path as written>: <ERRNO NAME>`, e.g. `EACCES`); unresolvable (`ENOENT`, `ENOTDIR`, `ENAMETOOLONG`, an unsearchable parent `EACCES`, a case-mismatched spelling on a case-sensitive volume, …) → `fileNotFoundError(shown)` (`mds::file_not_found`, `file not found: <path as written>`). `realpathParent` folds every canonicalize failure on the PARENT (`ENOENT`/`ENOTDIR`/`ELOOP`/`ENAMETOOLONG`/`EACCES`) into the same `fileNotFoundError`, and `closeModule` (wraps `handle.close()`) and the post-open `handle.stat()`/`readFile()` calls route their own failures through `readError` too — neither ever names the resolved absolute path, both key on `shown` (the path as written, escaped).
- **Symlinks** mirror `NativeFs::check_symlink_named`: canonicalize the parent (`realpathParent`), join the name as written, refuse by the final component's OWN file type (`O_NOFOLLOW` open → `ELOOP` only, caught before the generic classification above runs; the post-open `lstat` check covers where `O_NOFOLLOW` is 0, i.e. Windows, which also reports junctions). Never compare the canonical path with the written one — on case-insensitive volumes that misreports a case-mismatched name as a symlink (#408). Containment in the project root is decided on the canonical path.
- **`assertKeyMatchesDisk`** (9b63e32, #408) refuses an import whose virtual key names a different directory than the one `NativeFs` reads it from: the WASM engine resolves an import BY NAME from the importing module's key, while `NativeFs` resolves it ON DISK from the importing module's canonical directory — the two disagree exactly when the import leaves a symlinked directory through `..` (the OS applies `..` AFTER the link, the key-based resolution applies it before). Mismatch throws `mds::import` (`import path leaves a symlinked directory through '..', which the WASM backend cannot resolve: "<path>"`). The `visited` dedup set in `scan()` is now keyed by VIRTUAL KEY, not by resolved `absolutePath` — through a symlinked directory one on-disk file can sit under two keys, and the engine looks up each of them, so keying by path would silently skip the second.
...
- **Residuals (tracked in #414)**: the one remaining `security: …` error, `project root cannot be filesystem root`, is uncoded (it names no path); `normalizeVirtualKey`'s own escape/segment-cap/empty-key refusals and the `resource limit: …`-prefixed import-chain-depth/module-count/aggregate-size refusals are plain uncoded `Error`s too — none of them carries a path. A `PathError` carries no `help` field at all, while the native `mds::file_not_found` has one — so cross-backend tests compare `code` and `message` only.
- **Tests**: `__test__/forbidden-path-chars.spec.mjs` U-FP1–U-FP5 — the Rust↔JS differential over all 80 codepoints (predicate, entry keys, import strings, and `compileFile` through both backends incl. a symlink into a hostile-named directory); `__test__/scanner.spec.mjs` U-S11–U-S15 (normalizeVirtualKey), U-SM9–U-SM13/U-SM18 (#265 refusals), U-SM3/U-SM14–U-SM29 (#408 symlink, not-found and I/O shapes; U-SM21 asserts native and WASM agree on code and message; U-SM22–24 the symlinked-`..`-directory refusal and dual-key storage; U-SM25–29 the coded-error matrix — ELOOP/ENAMETOOLONG/EACCES-on-directory as file-not-found, EACCES-on-file as `mds::io`, and the hostile-working-directory case naming only the path as written; U-SM7/U-SM16/U-SM17 assert the symlink and escape refusals' exact code and message (U-SM7 also a relative entry, never the canonical dir); U-SM30 the lexical escape with an in-root control; U-SM31 a directory as import and entry; U-SM32 native-vs-WASM `compileFile` parity on a symlinked entry typed relative, a symlinked import, and both escapes, with a clean control). U-SM25 runs on Windows too (f0bcf7a): `helpers.mjs` `symlinkOrSkip` mirrors the Rust `make_symlink` — skip only on a Windows `EPERM` with `CI` unset, throw otherwise. The WASM side of a differential loads `crates/mds-wasm/pkg/` first — rebuild it from the current tree before running these; `__test__/helpers.mjs` gained shared assertion helpers for this matrix (a6d1673).
...
- `packages/mds/src/util/module-scanner.ts` — `buildModulesMap`, `normalizeVirtualKey`, `PathError`, `realpathParent`, `openNoFollow`, `cannotReadError`, `readError`, `symlinkError`, `escapesProjectError`, `closeModule`, `assertKeyMatchesDisk` (WASM-backend file pre-scanner)

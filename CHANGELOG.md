...
- `mds::is_forbidden_path_char` and `mds::escape_path_for_message`: the forbidden-path-character predicate (`is_control_char`'s 78-codepoint class plus `\n` and `\t` — 80 total) and its message-escaping helper (#265). The predicate is what the resolver, the built-in backends and the CLI now enforce (see **Changed**), and the contract a custom `FileSystem` backend that rewrites paths must apply.
- `mds::display_native_path`: the user-visible form of a filesystem path — strips a Windows verbatim `\\?\` prefix wherever that is lossless, a no-op off Windows. The single entry point a caller displaying a path (CLI status lines, bindings) should use; internal comparisons keep the canonical/verbatim form unchanged (#409).
...
- The 24 `mds-cli`/`mds-core` integration and unit tests that were `#[cfg(unix)]` only because they created a symlink now run on Windows too, via the same `make_symlink` helper (one copy per test binary/module, since `mds-cli`'s unit tests, its integration-test binaries, and `mds-core`'s unit and integration tests each compile separately): `crates/mds-cli/src/output.rs` (2), `crates/mds-cli/tests/{cli_build,cli_commands,cli_fmt,cli_source_map,cli_watch,dir_build,intrinsic_output,security}.rs` (18 across the eight files), `crates/mds-core/src/lib.rs` (1), and `crates/mds-core/tests/api_surface.rs` (1). One incidentally Unix-only-content test that had no genuine platform reason, `cli_fmt.rs`'s `bad_utf8_exits_two`, was un-gated alongside them. Every remaining `#[cfg(unix)]` gate in the touched files — Unix permission-mode-bit tests, chmod-provoked write-failure tests, `libc::kill`/`SIGINT` tests, and non-UTF-8-filename tests built via `OsStr(ing)Ext::from_bytes`/`from_vec` — now carries a one-line reason comment naming why the behavior it tests does not exist, or cannot be provoked the same way, on Windows (#147).

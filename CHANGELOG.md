...
- `mds::is_forbidden_path_char` and `mds::escape_path_for_message`: the forbidden-path-character predicate (`is_control_char`'s 78-codepoint class plus `\n` and `\t` — 80 total) and its message-escaping helper for #265. Additive only — nothing in this crate enforces the predicate yet; enforcement across the built-in backends, the resolver and the CLI lands in a follow-up commit.

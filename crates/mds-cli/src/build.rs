...
pub(crate) struct MdsConfig {
    ...
    /// Unknown rule NAMES: only `mds lint` warns on stderr (via `load_lint_config`) and
    /// continues. `mds build`, `check`, `fmt`, and `watch` load this field but do not
    /// emit the warning — an accepted asymmetry, not an oversight (see CHANGELOG).
    #[serde(default)]
...
/// NAMES: only `mds lint` warns on stderr and continues (via `load_lint_config`);
/// `mds build`, `check`, `fmt`, and `watch` deserialize this struct but do not
/// emit the warning — an accepted asymmetry, not an oversight (see CHANGELOG).
#[derive(Debug, Default, Deserialize)]
...
impl LintCliConfig {
    ...
    ///
    /// Uses [`mds::LintConfig::from_rules_checked`] so the caller cannot
    /// accidentally omit the unknowns check: the return type structurally forces
    /// the caller to decide what to do with the `Option<mds::UnknownRuleNames>`

...
- **Expected outcome:** The mutated workspace run MUST exit non-zero with exactly 11 `unfulfilled_lint_expectations` diagnostics — 10 anchored in fix.rs at the ten `#[expect(deprecated)]` fn-decl lines, and 1 anchored at the `F-API-3` test in api_surface.rs. The workspace command reports all 11 because removing the `#[deprecated]` attribute leaves the lib rlib compiling clean; both the lib-test (fix.rs `#[cfg(test)]`) and the integration-test (api_surface) targets become ready simultaneously, and cargo schedules them concurrently, so both targets emit their diagnostics (applies ADR-009). The workspace command does NOT abort before the api_surface target is compiled — that claim is build-scheduling dependent and not a property of the code. The restored run MUST exit 0. If the mutated run reports fewer than 11, the pin is weaker than claimed and AC-209-05 fails.
- **Verification method:** integration

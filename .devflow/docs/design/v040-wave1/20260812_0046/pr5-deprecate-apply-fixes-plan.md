...
3. **Suppression audit with a planted positive** — plant the multi-line block `#[expect(\n    deprecated,\n    reason = "positive control"\n)]` in `crates/mds-core/src/lexer.rs` (matching the attribute form shipped in this PR), run `grep -rn -A2 'allow(deprecated)\|expect(deprecated)\|#\[expect($' crates/ --include='*.rs'`, and confirm the `deprecated,` continuation line appears under the planted hit — proving the grep detects the multi-line form. Remove the block, re-run, and confirm the clean set is only fix.rs >1006 and `tests/api_surface.rs`. Read every hit; a bare count is not evidence. (BSD grep has no `-P` — basic alternation only; applies ADR-009, avoids PF-013.)
...
4. AC-209-04 (functionality, NEGATIVE): No `#[allow(deprecated)]` or `#[expect(deprecated, ...)]` attribute (in any syntactic form, including the multi-line `#[expect(\n    deprecated,\n    reason = "..."\n)]` form shipped in this PR) MUST appear in any file under `crates/*/src/` outside a `#[cfg(test)]` module — this prohibition explicitly includes text inside `///` doc comments and inside doc-test code fences. Occurrences are permitted at exactly three locations: (a) inside `crates/mds-core/src/lint/fix.rs` below line 1006, (b) inside `crates/mds-core/tests/`, and (c) `crates/mds-core/src/lint/config.rs` lines 287 and 289 — two `#[allow(deprecated)]` lines inside the compiled doctest for `LintConfig::from_rules`, which is already `#[deprecated]` on the wave base (present since #302, before this PR). The audit grep WILL return hits at (c); they are expected and pre-existing. Any hit outside (a), (b), or (c) is a regression introduced by this PR. The suppressions in this PR use the multi-line form; the audit grep must be multiline-aware — see test plan #4 for the exact command and the matching positive control (applies ADR-009, avoids PF-013). (Amends the original absolute prohibition per PF-015: an enumerated whitelist is less liable than an implicit completeness claim when pre-existing exceptions exist.)

...
5. **CHANGELOG `### Deprecated`** before `### Fixed` at 764. Public-facing copy: no em/en dashes, no placeholders.
6. **`tests/api_surface.rs` F-API-3** after 1418, with the `AD-209-2` docstring.
7. **Reverse mutation control (NEW, mandatory):** delete only the `#[deprecated(...)]` attribute, run `cargo clippy --workspace --all-targets -- -D warnings`, confirm **exactly 11** `unfulfilled_lint_expectations` (10 fix.rs + 1 api_surface.rs), then restore and confirm clean. Without this, AC-209-05 is an absence-only claim.
...
3. **Suppression audit with a planted positive** — plant `// allow(deprecated)` in `crates/mds-core/src/lexer.rs`, confirm `grep -rn 'allow(deprecated)\|expect(deprecated)' crates/ --include='*.rs'` finds it, remove it, re-run. Clean run must hit only fix.rs >1006 and `tests/api_surface.rs`. Read every hit; a bare count is not evidence. (BSD grep has no `-P` — basic alternation only.)
...
4. AC-209-04 (functionality, NEGATIVE): No file under `crates/*/src/` MUST contain the substring `allow(deprecated)` or `expect(deprecated)` outside a `#[cfg(test)]` module — this prohibition explicitly includes text inside `///` doc comments and inside doc-test code fences. Occurrences are permitted at exactly three locations: (a) inside `crates/mds-core/src/lint/fix.rs` below line 1006, (b) inside `crates/mds-core/tests/`, and (c) `crates/mds-core/src/lint/config.rs` lines 287 and 289 — two `#[allow(deprecated)]` lines inside the compiled doctest for `LintConfig::from_rules`, which is already `#[deprecated]` on the wave base (present since #302, before this PR). The audit grep WILL return hits at (c); they are expected and pre-existing. Any hit outside (a), (b), or (c) is a regression introduced by this PR. (Amends the original absolute prohibition per PF-015: an enumerated whitelist is less liable than an implicit completeness claim when pre-existing exceptions exist.)
5. AC-209-05 (functionality, positive, mutation-verified): With the implementation complete, deleting ONLY the `#[deprecated(...)]` attribute from `apply_fixes` (leaving the function and all suppressions in place) MUST cause `cargo clippy --workspace --all-targets -- -D warnings` to FAIL with exactly 11 `unfulfilled_lint_expectations` diagnostics — 10 in `crates/mds-core/src/lint/fix.rs` and 1 in `crates/mds-core/tests/api_surface.rs`. Restoring the attribute MUST restore a clean run.
6. AC-209-06 (functionality, positive control): At the intermediate state where `#[deprecated]` is attached but no suppression has been added, `cargo clippy -p mds-core --all-targets -- -D warnings` MUST report exactly 10 `deprecated` diagnostics, anchored at `crates/mds-core/src/lint/fix.rs` lines 1448, 1661, 1687, 1749, 1850, 1878, 1898, 1995, 2050, and 2392. Fewer than 10 MUST be treated as a failed run, not a clean one.
7. AC-209-07 (documentation, NEGATIVE): After the change, no rustdoc sentence or code comment under `crates/mds-core/src/` and no line in `.devflow/features/mds-lint/KNOWLEDGE.md` MUST instruct a caller to USE `apply_fixes`. Surviving mentions are permitted only as (a) text inside the deprecated item's own doc block, (b) a factual comparison between the two functions (fix.rs:624 and fix.rs:801), or (c) a Rust test identifier.
...
- **Setup:** Step 1 (positive control, mandatory first): temporarily insert the literal line `// allow(deprecated)` into `crates/mds-core/src/lexer.rs` and run `grep -rn 'allow(deprecated)\|expect(deprecated)' crates/ --include='*.rs'`. Step 2: remove the planted line and re-run the same grep. Note BSD grep has no `-P`; use only basic alternation as shown.
- **Expected outcome:** Step 1 MUST report the planted hit (proving the grep is live). Step 2 MUST report hits at exactly three locations: (a) `crates/mds-core/src/lint/fix.rs` lines greater than 1006, (b) inside `crates/mds-core/tests/api_surface.rs`, and (c) `crates/mds-core/src/lint/config.rs` lines 287 and 289. The (c) hits are pre-existing `#[allow(deprecated)]` inside the compiled doctest for the already-deprecated `LintConfig::from_rules`; they are present on the wave base before this PR (introduced by #302, confirmed by `git show wave/v0.4.0-wave1:crates/mds-core/src/lint/config.rs | grep -n 'allow(deprecated)'`). Any hit outside (a), (b), or (c) is a suppression leak introduced by this PR. Read every hit individually; a bare count is not acceptable evidence.
...
- **Setup:** On a clean tree at the final implementation state, delete only the `#[deprecated(...)]` attribute block above `pub fn apply_fixes` in `crates/mds-core/src/lint/fix.rs`. Run `cargo clippy --workspace --all-targets -- -D warnings`. Then `git checkout -- crates/mds-core/src/lint/fix.rs` and re-run.
- **Expected outcome:** The mutated run MUST exit non-zero with exactly 11 `unfulfilled_lint_expectations` diagnostics: 10 anchored in fix.rs at the ten `#[expect(deprecated)]` fn-decl lines, 1 anchored at the `F-API-3` test in api_surface.rs. The restored run MUST exit 0. If the mutated run reports fewer than 11, the pin is weaker than claimed and AC-209-05 fails.
- **Verification method:** integration
...
- **Setup:** Run `grep -n '^### ' CHANGELOG.md | head -20` and read the new section in full. Then run `grep -n '#NEW\|<OD-\|TBD\|XXX\|—\|–' CHANGELOG.md` scoped to the new hunk via `git diff <wave-base>..HEAD -- CHANGELOG.md`.
- **Expected outcome:** `### Deprecated` appears after `### Changed` (627) and immediately before `### Fixed` (764, shifted by the insert), inside `[Unreleased]`. Content names `mds::fix::apply_fixes`, `apply_fixes_incremental`, the `Fn`/`FnOnce` bound change, `FixOutcome::PartiallyFixed`, the removal version, and a numeric GitHub issue reference. Zero placeholder tokens and zero em/en dashes in the added lines.
- **Verification method:** manual
...
- **Expected outcome:** Exactly two hits. AD-209-1 in the apply_fixes rustdoc contains the literal phrase `applies ADR-004`, all three migration deltas, and a why-not-deleted statement that references the v0.3.0 tag absence. AD-209-2 on `F-API-3` explains the `#[expect]`-over-trybuild choice. Neither ID is written as `#209-1` (PF-010).
- **Verification method:** manual
...
- **Expected outcome:** `verify-versions.mjs` exits 0. The planted control is detected (proving the grep works on this shell — BSD grep quirks excluded). The clean run returns either zero hits (if `since` is omitted per OD-209-B) or exactly one hit whose value matches the version the wave will tag. If a `since` string is kept, RELEASING.md gains a pre-flight line for this grep.
- **Verification method:** manual

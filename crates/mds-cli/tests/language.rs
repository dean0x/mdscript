...
fn simple_variable_interpolation() {
    let result = mds::compile(fixture("simple.mds"), None)
        .unwrap()
        .into_markdown()
        .unwrap();
...
fn conditional_truthy() {
    let result = mds::compile(fixture("conditional.mds"), None)
        .unwrap()
        .into_markdown()
        .unwrap();
...
fn conditional_falsy() {
    let result = mds::compile(fixture("conditional_false.mds"), None)
        .unwrap()
        .into_markdown()
        .unwrap();
    assert!(!result.contains("Thanks for being premium!"));
...
#[test]
fn loop_over_array() {
    let result = mds::compile(fixture("loop.mds"), None)
        .unwrap()
        .into_markdown()
        .unwrap();
...
fn function_definition_and_call() {
    let result = mds::compile(fixture("function.mds"), None)
        .unwrap()
        .into_markdown()
        .unwrap();
    assert!(result.contains("Hello Alice!"));
...
#[test]
fn escaped_braces() {
    let result = mds::compile(fixture("escaped.mds"), None)
        .unwrap()
        .into_markdown()
        .unwrap();
...
fn code_block_passthrough() {
    let result = mds::compile(fixture("code_block.mds"), None)
        .unwrap()
        .into_markdown()
        .unwrap();
...
fn runtime_vars_override() {
    ...
    let result = mds::compile(fixture("simple.mds"), Some(vars))
        .unwrap()
        .into_markdown()
        .unwrap();
...
fn complete_example_welcome() {
    let result = mds::compile(fixture("welcome.mds"), None)
        .unwrap()
        .into_markdown()
        .unwrap();
    assert!(result.contains("Hello Alice!"));
...
#[test]
fn unicode_content() {
    let result = mds::compile(fixture("unicode.mds"), None)
        .unwrap()
        .into_markdown()
        .unwrap();
...
fn compile_str_no_frontmatter() {
    let result = mds::compile_str_with("Just plain text.", None, None)
        .unwrap()
        .into_markdown()
        .unwrap();
...
#[test]
fn nested_loops() {
    let result = mds::compile(fixture("nested_loops.mds"), None)
        .unwrap()
        .into_markdown()
        .unwrap();
...
fn function_called_in_loop() {
    let result = mds::compile(fixture("fn_in_loop.mds"), None)
        .unwrap()
        .into_markdown()
        .unwrap();
...
fn loop_var_shadows_outer() {
    let result = mds::compile(fixture("loop_var_shadow.mds"), None)
        .unwrap()
        .into_markdown()
        .unwrap();
...
fn function_param_shadows_outer() {
    let result = mds::compile(fixture("fn_param_shadow.mds"), None)
        .unwrap()
        .into_markdown()
        .unwrap();
    assert!(
...
fn nested_function_calls_in_interpolation() {
    let result = mds::compile(fixture("nested_fn_calls.mds"), None)
        .unwrap()
        .into_markdown()
        .unwrap();
...
fn multiple_escaped_braces() {
    let result = mds::compile(fixture("multiple_escaped_braces.mds"), None)
        .unwrap()
        .into_markdown()
        .unwrap();
...
#[test]
fn if_falsy_zero() {
    let result = mds::compile(fixture("if_falsy_zero.mds"), None)
        .unwrap()
        .into_markdown()
        .unwrap();
    assert!(
...
#[test]
fn if_falsy_null() {
    let result = mds::compile(fixture("if_falsy_null.mds"), None)
        .unwrap()
        .into_markdown()
        .unwrap();
    assert!(
...
fn if_falsy_empty_string() {
    let result = mds::compile(fixture("if_falsy_empty_string.mds"), None)
        .unwrap()
        .into_markdown()
        .unwrap();
    assert!(
...
fn if_falsy_empty_array() {
    let result = mds::compile(fixture("if_falsy_empty_array.mds"), None)
        .unwrap()
        .into_markdown()
        .unwrap();
    assert!(
...
fn if_falsy_boolean_false() {
    ...
    let result = mds::compile(fixture("if_falsy_false.mds"), None)
        .unwrap()
        .into_markdown()
        .unwrap();
    assert!(
...
fn multi_param_function() {
    ...
    let result = mds::compile(fixture("multi_param.mds"), None)
        .unwrap()
        .into_markdown()
        .unwrap();
    assert!(
...
fn single_quote_string_literal_in_function_args() {
    ...
    let result = mds::compile(fixture("single_quote_args.mds"), None)
        .unwrap()
        .into_markdown()
        .unwrap();
    assert!(
...
fn md_file_with_type_mds_compiles() {
    ...
    let result = mds::compile(fixture("type_mds_md_file.md"), None)
        .unwrap()
        .into_markdown()
        .unwrap();
    assert!(
...
fn function_calls_function() {
    let result = mds::compile(fixture("fn_calls_fn.mds"), None)
        .unwrap()
        .into_markdown()
        .unwrap();
...
fn nested_conditionals() {
    let result = mds::compile(fixture("nested_if.mds"), None)
        .unwrap()
        .into_markdown()
        .unwrap();
...
fn elseif_mixed_equality_and_inequality_in_chain() {
    ...
    let source2 = "---\nrole: visitor\n---\n\
        ...
        @else:\nanon_visitor\n\
        @end\n";
    let result2 = mds::compile_str(source2).unwrap().into_markdown().unwrap();
    assert!(
